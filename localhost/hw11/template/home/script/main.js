...
window.onload = function(){
    ...
    contentFeed.addEventListener('click', (e) => {
        if (e.target.tagName === 'BUTTON' && e.target.textContent === 'ещё') {
            e.target.textContent = 'свернуть';
            e.target.previousElementSibling.firstElementChild.hidden = true;
            e.target.previousElementSibling.firstElementChild.nextElementSibling.hidden = false;
        } 
        else if (e.target.tagName === 'BUTTON' && e.target.textContent === 'свернуть'){
            e.target.textContent = 'ещё';
            e.target.previousElementSibling.firstElementChild.hidden = false;
            e.target.previousElementSibling.firstElementChild.nextElementSibling.hidden = true;
        }
